"""Run bounded Sprint 18 native material-foundry acceptance and visual QA."""
...
def _require_success(results) -> None:
    ...
    if failures:
        raise RuntimeError(f"Sprint 18 acceptance command failed: {failures}")
...
def main() -> None:
    ...
    render = artifact_dir / "observations" / "sprint18_live_karma_cpu.png"
    visual = None
    critique = None
    if arguments.render:
        visual = artifact_dir / "manifests" / "sprint18_live_visual_verification.json"
        visual_result = _dispatch(
            dispatcher,
            CommandEnvelope(
                tool="visual.analyze",
                request_id="sprint18-live-visual",
                arguments={
        ...
        critique = artifact_dir / "manifests" / "sprint18_live_critique_packet.json"
        critique_result = _dispatch(
            dispatcher,
            CommandEnvelope(
                tool="verification.critique.package",
                request_id="sprint18-live-critique",
                arguments={
                    "image_paths": [str(render)],
                    "graph_path": str(
                        artifact_dir / "observations" / "sprint18_live_lop_graph.svg"
                    ),
                    "validation_paths": [
                        ...
                        str(
                            artifact_dir
                            / "manifests"
                            / "sprint18_live_material_foundry_manifest.json"
                        ),
                        str(visual),
                    ],
                    "code_paths": [
                        ...
                        str(
                            ...
                            / "recipes"
                            / "cop"
                            / "procedural_material_foundry.yaml"
                        ),
                        str(
                            Path(__file__).resolve().parents[1]
                            / "skills"
    ...
    validation_path = artifact_dir / "manifests" / "sprint18_live_pbr_channels.json"
        ...
        json.dumps(
            {
                ...
                "resolution": validation["spec"]["resolution"],
                "candidates": [item["id"] for item in validation["candidates"]],
                "channels": validation["spec"]["channels"],
