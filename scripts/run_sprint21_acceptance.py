"""Run Sprint 21's capability-gated SideFX Labs World Seed acceptance."""
...
def _require_success(results) -> None:
    ...
    if failures:
        raise RuntimeError(f"Sprint 21 acceptance command failed: {failures}")


def main() -> None:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--artifact-dir", required=True)
    parser.add_argument("--terrain-samples", type=int, default=96)
    parser.add_argument("--labs-available", action=argparse.BooleanOptionalAction, default=True)
    ...
    skill = load_skill("skills/world.world_seed_atlas_labs")
    calls = skill.plan(
        artifact_dir=str(artifact_dir),
        run_id="sprint21_live",
        base_seed=19019,
        terrain_samples=arguments.terrain_samples,
        world_size=9.0,
        labs_available=arguments.labs_available,
        width=768,
        height=432,
        frame=1.0,
        time_limit=120.0,
    ...
    render = artifact_dir / "observations" / "sprint21_live_karma_cpu.png"
    visual = None
    critique = None
    if arguments.render:
        visual = artifact_dir / "manifests" / "sprint21_live_visual_verification.json"
        visual_result = _dispatch(
            dispatcher,
            CommandEnvelope(
                tool="visual.analyze",
                request_id="sprint21-live-visual",
                arguments={
        ...
        critique = artifact_dir / "manifests" / "sprint21_live_critique_packet.json"
        critique_result = _dispatch(
            dispatcher,
            CommandEnvelope(
                tool="verification.critique.package",
                request_id="sprint21-live-critique",
                arguments={
                    "image_paths": [str(render)],
                    "graph_path": str(artifact_dir / "observations" / "sprint21_live_obj_graph.svg"),
                    "validation_paths": [
                        str(artifact_dir / "manifests" / "sprint21_live_labs_atlas_validation.json"),
                        str(artifact_dir / "manifests" / "sprint21_live_labs_atlas_manifest.json"),
                        str(visual),
                    ],
                    "code_paths": [
                        str(Path(__file__).resolve()),
                        str(Path(__file__).resolve().parents[1] / "hermes_houdini" / "labs_atlas.py"),
                        str(Path(__file__).resolve().parents[1] / "recipes" / "sop" / "world_seed_labs_enhancement.yaml"),
                        str(Path(__file__).resolve().parents[1] / "skills" / "world.world_seed_atlas_labs" / "skill.py"),
                    ],
    ...
    validation_path = artifact_dir / "manifests" / "sprint21_live_labs_atlas_validation.json"
