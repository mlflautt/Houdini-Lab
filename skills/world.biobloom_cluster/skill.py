...
def plan(parent_node_id: str, seed: int = 42, cluster_count: int = 24,
         growth_bias: float = 0.65) -> list[dict[str, Any]]:
    calls: list[dict[str, Any]] = []
    try:
        scatter = load_recipe(_RECIPES / "sop" / "scatter_cluster_points.yaml")
        calls += scatter.render(parent_node_id, count=cluster_count, seed=seed)
    except Exception:
        # fallback to direct node.create if pyyaml unavailable
        calls.append(build_envelope("node.create", {
            "parent_path": parent_node_id,
            "operator_type": "scatter",
            "name": "SCATTER_PTS",
            "role": "bloom_seeds",
            "parameters": {"force_total": cluster_count, "seed": seed},
        }).as_dict())
    calls.append(build_envelope("node.create", {
        "parent_path": parent_node_id,
        "operator_type": "null",
        "name": "OUT_GEO",
        "role": "output",
        "comment": f"Biobloom cluster (growth_bias={growth_bias})",
    }).as_dict())
    return calls
